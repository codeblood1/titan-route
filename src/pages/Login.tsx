...
export default function Login() {
  ...
  const testConnection = async () => {
    if (!supabase) {
      setError("Supabase client not initialized. Check your URL and Anon Key configuration.");
      return;
    }
    setError("");
    setIsLoading(true);
    try {
      ...
      const { error: testError } = await supabase.from("packages").select("id", { count: "exact", head: true });
      const ms = Date.now() - start;

      if (testError) {
        if (testError.message?.includes("JWT") || testError.message?.includes("invalid")) {
          setError(`Invalid Anon Key. (${ms}ms) Check that your Supabase Anon Key is correct.`);
        } else if (testError.code === "PGRST301") {
          setError(`Database unreachable. (${ms}ms) Your Supabase project may be paused. Go to Supabase Dashboard and click "Restore".`);
        } else if (testError.message?.includes("Failed to fetch") || testError.message?.includes("Network")) {
          setError(`Network error — cannot reach Supabase. (${ms}ms) Check your internet and Project URL.`);
        } else {
          setError(`Supabase error: ${testError.message} (${testError.code}, ${ms}ms)`);
        }
      } else {
        setError(`Supabase is reachable! (${ms}ms) Connection OK. Try logging in now.`);
      }
    } catch (err: any) {
      const msg = err?.message || String(err);
      if (msg.includes("timed out")) {
        setError("Connection timed out. Your Supabase project may be paused or the URL is wrong.");
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.reason === "wrong_password") {
      ...
      setError(
        ...
        `Assign an admin role in the admin_roles table.`
      );
    } else if (result.reason === "supabase_error") {
      const debug = result.debug || "";
      if (debug.includes("timed out")) {
        setError(
          `Connection timed out. Common causes:\n` +
          `1. Supabase project is paused (free tier auto-pauses after 7 days)\n` +
          `2. Wrong Project URL or Anon Key\n` +
          `3. Network/firewall blocking connection\n\n` +
          `Click "Test Supabase Connection" above to diagnose.`
        );
      } else {
        setError(`Supabase error: ${debug}`);
      }
    } else {
      setError(`Login failed: ${result.debug || result.reason || "unknown error"}`);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-blue-900 flex items-center justify-center px-4 py-8">
      <div className="w-full max-w-lg">
            ...
            {!hasSupabase && !hasStoredConfig && (
              <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg flex items-start gap-2">
                ...
                <div className="text-xs text-amber-700">
                  ...
                  <p>Enter your Supabase credentials below to connect to your live backend.</p>
                </div>
              </div>
            )}
        ...
        <p className="text-center text-xs text-blue-300/60 mt-6">
          Production backend via Supabase
        </p>
      </div>
    </div>
  );
